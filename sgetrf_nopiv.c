...
void sgetrf_nopiv(int m, int n, float *A, int lda) {
    ...
    if (nb > m || nb > n) {
        ...
        for (j = 0; j < min_mn; j += nb) {
            ...
            // Factor panel
            if (j != 0) {
                cublasGetMatrix(m, jb, sizeof(float), &dA(0, j), ldda, &A(0, j),
                                lda);
            }
            sgetrf2_nopiv(m - j, jb, &A(j, j), lda);

            if (j + jb < n) {
                cublasSetMatrix(m - j, jb, sizeof(float), &A(j, j), lda,
                                &dA(j, j), ldda);
                cublasStrsm(handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER,
                            CUBLAS_OP_N, CUBLAS_DIAG_UNIT, jb, n - j - jb, &one,
                            &dA(j, j), ldda, &dA(j, j + jb), ldda);

                // strsm('L', 'L', 'N', 'U', jb, n-j-jb, 1.0, &A(j, j), lda,
                // &A(j, j+jb), lda);

                if (j + jb < m) {
                    cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, m - j - jb,
                                 n - j - jb, jb, &none, &dA(j + jb, j),
                                 CUDA_R_32F, ldda, &dA(j, j + jb), CUDA_R_32F,
                                 ldda, &one, &dA(j + jb, j + jb), CUDA_R_32F,
                                 ldda, CUDA_R_32F, CUBLAS_GEMM_DFALT_TENSOR_OP);
                    // cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m-j-jb,
                    // n-j-jb, jb,
                    //        &none, &dA(j+jb, j), ldda, &dA(j, j+jb), ldda,
                    // &one, &dA(j+jb, j+jb), ldda);
                    // sgemm('N', 'N', m-j-jb, n-j-jb, jb, -1.0, &A(j+jb, j),
                    // lda, &A(j, j+jb), lda, 1.0,
                    //     &A(j+jb, j+jb), lda);
                }
            }
        }
